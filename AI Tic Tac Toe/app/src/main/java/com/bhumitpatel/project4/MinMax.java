...
public class MinMax {
	 ...
	 * @param init
	 */
	MinMax(String[] init, String s, int i )		
	{
		initState = init;			//set the initial state string array
		choice = s;
		
		setInitNode();				//set the initial state node	
		stateList = createStateList(s, initNode);		//get list of all X moves from initial state	
		setStateList_MinMaxValues(stateList, i);			//set initial state minMax values for X moves				
	}
	...
	private ArrayList<Node> createStateList(String minOrMax, Node stateNode)
	{
		...
		if(minOrMax.equals("max"))		//list of all X moves
		{
